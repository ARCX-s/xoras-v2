...
let scheduleSlots={}; // { 'Понедельник': [{para,time,date,lessons:[]}] }
...
async function loadGroupSchedule(groupName){
  ...
  try{
    ...
    // Выбираем сегодня или первый доступный день
...
function buildDayTabs(){
  ...
  // Определяем тип недели из первого доступного слота
  let weekType='';
  ...
  // Показываем индикатор недели над вкладками
...
$('sinput').oninput=function(){
  ...
  // Поиск по загруженному расписанию текущей группы
...
(function init(){
  ...
  if(savedGroup&&savedGroupName){
    ...
    loadGroupSchedule(savedGroupName).then(()=>{
      if(savedDay&&scheduleSlots[savedDay]){
        selDay=savedDay;
      }
      buildDayTabs();
      renderSchedule();
